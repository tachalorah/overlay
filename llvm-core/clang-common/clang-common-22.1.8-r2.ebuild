...
IUSE="
	...
	bootstrap-prefix cet emacs gcc-install hardened llvm-libunwind transitive-headers
"
...
src_install() {
	...
	if ! use transitive-headers ; then
		cat >> "${ED}/etc/clang/gentoo-hardened.cfg" <<-EOF || die
			# Options below are conditional on USE=transitive-headers
...
pkg_preinst() {
	if has_version -b sys-devel/gcc-config && has_version sys-devel/gcc && use gcc-install
	then
